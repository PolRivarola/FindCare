...
function pickUsername(u: number | { id: number; username: string; first_name?: string; last_name?: string }) {
  if (typeof u === "number") return { id: u, label: `Usuario ${u}` };
  const full = [u.first_name, u.last_name].filter(Boolean).join(" ").trim();
  return { id: u.id, label: full || u.username };
}
...
export function mapServiciosToUI(rows: ServicioDTO[]): Solicitud[] {
  return rows.map((r) => {
    ...
    return {
      ...
      hora: r.horas_dia || "—",
      ubicacion: "—",                                 // no existe en el modelo actual
      rangos_horarios: [],
